import { ActionIcon, AspectRatio, Card, Group, Image, LoadingOverlay, rem, Text } from '@mantine/core';
...
import { Icon3dRotate, IconLayersSubtract } from '@tabler/icons-react';
...
export function ModelCard({ projectUuid, asset, selected, onSelectChange, view3d, onView3dChange, onDelete }: AssetCardProps) {
    ...
    const sendToSlicerURL = `prusaslicer://open?file=${encodeURI(`${local_backend}/projects/${projectUuid}/assets/${asset?.sha1}`)}`;
    return (
        <Card withBorder padding="lg" radius="md" className={classes.card} style={{ minWidth: size, width: size, borderColor: selected ? 'red' : '' }} >
            ...
            <Card.Section className={classes.footer}>
                <Group justify="flex-end">
                    <Group gap={0}>
                        <ActionIcon variant="subtle" color="gray" component='a' href={sendToSlicerURL}>
                            <IconLayersSubtract
